...
class Random_BLS {
  ...
  def pertub_dir(VertexCollect:ListBuffer[Long],EdgeCollection:ListBuffer[(Long,Long)],clique:ListBuffer[Long],tabuList:Map[Long,Int],penaltyList:Map[Long,Int], clique_best:ListBuffer[Long]): Unit ={
    val loop = new Breaks
    ...
    val v = clique((new Random).nextInt(clique.size))
  ...
  def BLS(VertexCollect:ListBuffer[Long],EdgeCollection:ListBuffer[(Long,Long)],clique_best:ListBuffer[Long],clique:ListBuffer[Long],tabuList:Map[Long,Int],penaltyList:Map[Long,Int],counter_local:Int):Int ={
    val loop = new Breaks;
    var clique_local = new ListBuffer[Long]
    var counter_local_Random = counter_local
    ...
    while(numsteps<maxsteps){
      ...
      if(clique.size>clique_best.size){
        ...
        counter_local_Random = 0
      }
      else{counter_local_Random+=1}
      if(counter_local_Random>3){
        ...
        if (k<=p){
          var i=0
          while(i<perturbSte){
            pertub_dir(VertexCollect,EdgeCollection,clique,tabuList,penaltyList,clique_best)
            Update(tabuList)
            i=i+1
          }
          clique_local.clear()
